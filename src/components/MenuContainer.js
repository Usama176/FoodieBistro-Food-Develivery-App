import React, { useEffect, useState } from "react";
import { IoFastFood } from "react-icons/io5";
import { categories } from "../utils/data";
import { motion } from "framer-motion";
import { useStateValue } from "../context/StateProvider";
import FoodItemList from './FoodItemList';

const MenuContainer = () => {
  // State to keep track of the selected filter
  const [filter, setFilter] = useState("chicken");

  // Retrieve food items and dispatch from context
  const [{ foodItems }, dispatch] = useStateValue();

  return (
    <section className="w-full my-8" id="menu">
      <div className="w-full flex flex-col items-center justify-center">
        {/* Title with decorative underline */}
        <p className="text-2xl font-semibold capitalize text-headingColor relative 
          before:absolute before:rounded-lg before:content before:w-16 before:h-1 
          before:-bottom-2 before:left-0 before:bg-gradient-to-tr from-orange-400 
          to-orange-600 transition-all ease-in-out duration-100 mr-auto"
        >
          Our Hot Dishes
        </p>

        {/* Filter Categories */}
        <div className="w-full mt-5 flex items-center justify-start lg:justify-center 
          gap-5 py-6 overflow-x-scroll scrollbar-none"
        >
          {categories &&
            categories.map((category) => (
              <motion.div
                whileTap={{ scale: 0.75 }}
                key={category.id}
                className={`group ${filter === category.urlParamName
                  ? "bg-cartNumBg"
                  : "bg-card"
                  } w-24 min-w-[94px] h-28 cursor-pointer rounded-lg drop-shadow-xl 
                  flex flex-col gap-3 items-center justify-center hover:bg-cartNumBg `}
                onClick={() => setFilter(category.urlParamName)}
              >
                {/* Category Icon */}
                <div
                  className={`w-10 h-10 rounded-full shadow-lg ${
                    filter === category.urlParamName
                      ? "bg-white"
                      : "bg-cartNumBg"
                    } group-hover:bg-white flex items-center justify-center`}
                >
                  <IoFastFood
                    className={`${
                      filter === category.urlParamName
                        ? "text-textColor"
                        : "text-white"
                      } group-hover:text-textColor text-lg`}
                  />
                </div>
                {/* Category Name */}
                <p
                  className={`text-sm ${
                    filter === category.urlParamName
                      ? "text-white"
                      : "text-textColor"
                    } group-hover:text-white`}
                >
                  {category.name}
                </p>
              </motion.div>
            ))}
        </div>

        {/* Display food items based on the selected filter */}
        <div className="w-full">
          <FoodItemList
            flag={false}
            data={foodItems?.filter((e) => e.category === filter)}
          />
        </div>
      </div>
    </section>
  );
};

export default MenuContainer;